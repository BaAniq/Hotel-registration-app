...
def check_in_guest():
    ...
    room_label = Label(text='Last name', font=('Calibre', 10), bg=beige, padx=3)
    room_label.grid(column=2, row=3)

    room_textbox = Entry(width=12)
    room_textbox.grid(column=3, row=3)

    labels_to_delete = [back_button, name_label, name_textbox, last_name_label, last_name_textbox, room_label, room_textbox]
def destroy_view():
    global labels_to_delete
